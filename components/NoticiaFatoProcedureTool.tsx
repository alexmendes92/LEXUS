...
import { FileSearch, Upload, Send, FileText, Loader2, Sparkles, Copy, CheckCircle, Paperclip, X, Trash2, ArrowRight, PhoneOutgoing, UserCircle, MessageSquare, FolderOpen, AlertCircle } from 'lucide-react';
  ...
  const handleGenerate = async (refinementInstruction?: string) => {
    ...
    if (!isRefinement) {
        if (procedureFiles.length === 0 && contactFiles.length === 0 && victimFiles.length === 0 && !additionalText.trim()) {
            setFeedbackError("Por favor, forneça o procedimento inicial ou outros documentos para análise.");
            return;
        }
    ...
    try {
      ...
      console.error(error);
      setFeedbackError(error?.message || "Erro ao processar. Verifique os arquivos anexados.");
    } finally {
